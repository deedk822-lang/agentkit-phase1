...
# Production Control Plane Setup Script
# Complete GCP/GKE cluster setup with Workload Identity
...
setup_configuration() {
    ...
    if [ ! -f .env ]; then
        log_warning ".env file not found. Please copy .env.template to .env and fill it out."
        exit 1
    fi
    set -a
    source .env
...
setup_gcp_cluster() {
    ...
    if ! confirm "Have you authenticated with GCP? (run: gcloud auth login)"; then
        gcloud auth login
    fi
...
main() {
    ...
    log_info "You can now deploy the applications using: kubectl apply -f k8s/gcp-deployment.yml"
}
